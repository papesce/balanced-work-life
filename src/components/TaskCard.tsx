...
export function TaskCard({ task, onComplete, onUpdate, onDelete }: TaskCardProps) {
  ...
  return (
    <div className={`bg-white rounded-lg border-l-4 ${categoryColor} p-4 shadow-sm flex items-start gap-3`}>
      {task.status === "active" && (
        <button
          onClick={() => onComplete(task.id)}
          className="mt-0.5 w-5 h-5 border-2 border-gray-300 rounded-full hover:border-indigo-500 flex-shrink-0 transition-colors"
